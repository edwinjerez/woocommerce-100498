...
class Cartflows_Pro_Checkout_Markup {
	...
	public function __construct() {
		...
		add_filter( 'cartflows_show_coupon_field', array( $this, 'show_hide_coupon_field_on_checkout' ) );
	...
	function prepare_checkout_fields_lite( $fields, $original_fields, $checkout_id ) {
		...
		if ( is_array( $fields ) && ! empty( $fields ) ) {
			foreach ( $fields as $name => $field ) {
				$is_field = wcf()->options->get_checkout_meta_value( $checkout_id, 'wcf-' . $name );

				$field_widths = wcf()->options->get_checkout_meta_value( $checkout_id, 'wcf-field-width_' . $name );
	 ...
	 * @return bool
	 */
	function show_hide_coupon_field_on_checkout( $is_field ) {
