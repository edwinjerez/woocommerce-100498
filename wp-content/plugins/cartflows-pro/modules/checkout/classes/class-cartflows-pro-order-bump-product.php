...
class Cartflows_Pro_Order_Bump_Product {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_checkout_update_order_review', array( $this, 'order_bump_process' ) );
	 ...
	 * Process bump order.
	 *
	 * @param string $post_string post data string.
	 */
	function order_bump_process( $post_string ) {
		...
		if ( null != $found_item_key && 'true' === $checked ) {
			...
			if ( $discount_enabled && 'coupon' !== $discount_type ) {
				...
				if ( 'yes' === $is_variable || 'yes' === $is_variation ) {
					WC()->cart->add_to_cart( $parent_id, 1, $product_id, array(), $cart_item_data );
				} else {
