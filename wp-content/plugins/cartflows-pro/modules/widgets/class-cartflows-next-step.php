...
class Cartflows_Next_Step extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			...
			__( 'CartFlows Next Step', 'cartflows-pro' ),
			// Widget description.
			array( 'description' => __( 'Next Step Widgets', 'cartflows-pro' ) )
		);
	}
	...
	function widget( $args, $instance ) {
		...
		if ( ! $step_id || ! $flow_id ) {
			...
			if ( $post && CARTFLOWS_STEP_POST_TYPE === $post->post_type ) {
				...
				$flow_id = intval( get_post_meta( $step_id, '_flow_id', true ) );
			}
		}
		$output = 'No Data';
		...
		if ( $flow_id ) {
			...
			if ( is_array( $steps ) && ! empty( $steps ) ) {
				...
				if ( $navigation && is_array( $navigation ) ) {
					...
					$output = '<div class="wcf-step-navigation"><a class="button" target="_self" href="' . get_permalink( $navigation['id'] ) . '">' . __( 'Next Step', 'cartflows-pro' ) . '</a></div>';
				}
			}
		}
	...
	function form( $instance ) {
		...
		if ( isset( $instance['title'] ) ) {
			...
			$title = __( 'New title', 'cartflows-pro' );
		}
		...
		?>
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'cartflows-pro' ); ?></label> 
			<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'flow_id' ); ?>"><?php _e( 'Flow ID:', 'cartflows-pro' ); ?></label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'step_id' ); ?>"><?php _e( 'Step ID:', 'cartflows-pro' ); ?></label> 
