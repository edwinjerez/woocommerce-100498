...
if ( is_woocommerce_active() ) {
	...
	class WC_USPS {
		...
		public function plugin_row_meta( $links, $file ) {
			if ( $file === plugin_basename( __FILE__ ) ) {
				$row_meta = array(
					...
					'support' => '<a href="' . esc_url( apply_filters( 'woocommerce_hipping_usps_support_url', 'https://woocommerce.com/my-account/create-a-ticket/?select=18657' ) ) . '" title="' . esc_attr( __( 'Open a support request at WooCommerce.com', 'woocommerce-shipping-usps' ) ) . '">' . __( 'Support', 'woocommerce-shipping-usps' ) . '</a>',
				);
		...
		public function init() {
			require_once dirname( __FILE__ ) . '/includes/class-wc-usps-privacy.php';
