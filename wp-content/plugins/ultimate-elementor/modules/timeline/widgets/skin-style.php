...
class Skin_Style {
	...
	public function render_excerpt( $settings ) {
		...
		add_filter( 'excerpt_length', [ $this, 'uael_timeline_excerpt_length' ], 20 );
		add_filter( 'excerpt_more', [ $this, 'uael_timeline_excerpt_more' ], 20 );
	...
	public function render_date( $settings ) {
		...
		echo apply_filters( 'uael_timeline_the_date_format', get_the_date(), get_option( 'date_format' ), '', '' );
	}
	...
	public function render_pagination( $settings, $query, $query_obj ) {
		...
		if ( $total_pages > 1 ) {
			...
			?>
			<nav class="uael-timeline-pagination" <?php echo $class; ?> role="navigation" aria-label="<?php _e( 'Pagination', 'uael' ); ?>">
		...
		<div <?php echo $this->get_render_attribute_string( 'timeline_main' ); ?>>
			<div <?php echo $this->get_render_attribute_string( 'timeline_days' ); ?>>
				...
				while ( $query->have_posts() ) {
					...
					if ( ( '' != $settings['timeline_all_icon'] ) ) {
						$this->add_render_attribute( 'card_' . $post_id, 'class', $settings['timeline_all_icon'] );
					}
					...
					?>
					<div <?php echo $this->get_render_attribute_string( 'current_' . $post_id ); ?>>
						<div <?php echo $this->get_render_attribute_string( 'data_alignment' . $post_id ); ?>>
							<div class="uael-timeline-marker">
								<i <?php echo $this->get_render_attribute_string( 'card_' . $post_id ); ?>></i>
							</div>

							<div <?php echo $this->get_render_attribute_string( 'timeline_alignment' . $post_id ); ?>>
								<div <?php echo $this->get_render_attribute_string( 'timeline_events' . $post_id ); ?>>
									<?php if ( 'module' == $settings['post_timeline_cta_type'] ) { ?>
										<a href="<?php the_permalink(); ?>">
									<?php } ?>
										<div <?php echo $this->get_render_attribute_string( 'timeline_events_inner' . $post_id ); ?>>

											<?php if ( 'yes' == $settings['post_thumbnail'] && '' != get_the_post_thumbnail_url( $post_id ) ) { ?>
												<div class="uael-timeline-featured-img">
													<?php echo $this->render_featured_image( $settings ); ?>
												</div>
											<?php } ?>
											<div class="uael-timeline-date-hide uael-date-inner">
												<div class="inner-date-new">
													<?php if ( '' == $settings['post_timeline_date_type'] ) { ?>
														<p><?php echo $this->render_date( $settings ); ?></p>
													<?php } elseif ( 'updated' == $settings['post_timeline_date_type'] ) { ?>
														<p><?php echo get_the_modified_date( '', $post_id ); ?></p>
											...
											<div <?php echo $this->get_render_attribute_string( 'timeline_content' . $post_id ); ?>>
												<?php if ( 'yes' == $settings['post_title'] && '' != get_the_title( $post_id ) ) { ?>
													<div class="uael-timeline-heading-text">
														<<?php echo $settings['timeline_heading_tag']; ?> <?php echo $this->get_render_attribute_string( 'heading_setting_key' ); ?>><?php echo $this->render_title(); ?></<?php echo $settings['timeline_heading_tag']; ?>>
													</div>
												<?php } ?>
												<?php if ( 'yes' == $settings['post_excerpt'] ) { ?>
													<div class="uael-timeline-desc-content"><?php echo $this->render_excerpt( $settings ); ?></div>
												<?php } ?>
												...
												<?php if ( 'link' == $settings['post_timeline_cta_type'] ) { ?>
													<div class="uael-timeline-link-style">
														<a href="<?php the_permalink(); ?>" class="uael-timeline-link">
															<span><?php echo $dynamic['post_timeline_link_text']; ?></span>
														</a>
													</div>
												<?php } ?>
											</div>
