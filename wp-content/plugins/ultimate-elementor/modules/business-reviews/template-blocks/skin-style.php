	...
	public function get_google_api( $settings ) {
		...
		if ( '' != $integration_options['google_places_api'] ) {
			...
			if ( 'yes' === self::$google_api_status ) {
				...
				echo __( '<span class="uael-reviews-notice-message"><span class="uael-reviews-error-message">Google Error Message:</span> Incorrect Google API key! Please set up the API key from UAE settings.</span>', 'uael' );
				return false;
			}
		} elseif ( $is_editor ) {
			echo __( '<span class="uael-reviews-notice-message"><span class="uael-reviews-error-message">Google Error Message:</span> Please configure the Google API key to display the reviews.</span>', 'uael' );
			return false;
		} else {
			return false;
		}
		...
		if ( false === $result ) {
			...
			if ( is_wp_error( $result ) ) {
				...
				if ( $is_editor ) {
					...
					echo sprintf( __( '<span class="uael-reviews-notice-message">Something went wrong while fetching Google reviews: %1$s</span>', 'uael' ), $error_message );
				}
				return;
			}
	...
	public function get_yelp_api( $settings ) {
		...
		if ( '' !== $business_id ) {
			...
			if ( '' != $integration_options['yelp_api'] ) {
				...
				echo __( '<span class="uael-reviews-notice-message"><span class="uael-reviews-error-message">Yelp Error Message:</span> Please set the Yelp API key to display the reviews.</span>', 'uael' );
				return false;
			} else {
				return false;
			}
			...
			if ( is_wp_error( $result ) ) {
				...
				if ( $is_editor ) {
					/* translators: %1$s doc link */
					echo sprintf( __( '<span class="uael-reviews-notice-message">Something went wrong: %1$s</span>', 'uael' ), $error_message );
				}
				delete_transient( $transient_name );
				return;
			}
			...
			if ( 200 !== $response_code && ! empty( $response_message ) ) {
				if ( $is_editor ) {
					...
					if ( 'VALIDATION_ERROR' === $error_code ) {
						echo __( '<span class="uael-reviews-notice-message"><span class="uael-reviews-error-message">Yelp Error Message:</span> Incorrect Yelp API key. Please set up the API key from UAE settings.</span>', 'uael' );
					} elseif ( 'BUSINESS_NOT_FOUND' === $error_code ) {
						echo __( '<span class="uael-reviews-notice-message"><span class="uael-reviews-error-message">Yelp Error Message:</span> Incorrect Business ID.</span>', 'uael' );
					}
				}
				delete_transient( $transient_name );
				return;
				...
				if ( $is_editor ) {
					/* translators: %1$s doc link */
					echo sprintf( __( '<span class="uael-reviews-notice-message"> %1$s Unknown error occurred.</span>', 'uael' ), $response_code );
				}
				delete_transient( $transient_name );
				return;
			}
	...
	public function get_reviewer_name( $review, $settings ) {
		if ( 'yes' === $this->get_instance_value( 'reviewer_name' ) ) { ?>
			<?php if ( 'yes' === $this->get_instance_value( 'reviewer_name_link' ) ) { ?>
				<span class="uael-reviewer-name"><?php echo "<a href={$review['author_url']} target='_blank'>{$review['author_name']}</a>"; ?></span>
			<?php } else { ?>
				<span class="uael-reviewer-name"><?php echo "{$review['author_name']}"; ?></span>
				<?php
			}
		}
	}
...
<g>
			...
			<div class="uael-review-details">
				...
				<?php if ( 'yes' === $this->get_instance_value( 'review_rating' ) ) { ?>
					<span class="elementor-star-rating__wrapper">
						<span class="uael-star-rating"><?php echo $this->render_stars( $total_rating, $review, $settings ); ?></span>
					</span>
				<?php } ?>
				<?php
				if ( 'yes' === $this->get_instance_value( 'review_date' ) ) {
					...
					?>
					<span class="uael-review-time"><?php echo $date_value . $via_source; ?></span>
				<?php } ?>
				<?php
	...
	public function render( $style, $settings, $node_id ) {
		...
		$this->add_render_attribute(
			'uael-reviews-parent-schema',
			[
				'class' => 'uael-business-reviews-widget',
			]
		);

		if ( 'yes' === $settings['rating_schema_switch'] ) {
			$this->add_render_attribute(
				'uael-reviews-parent-schema',
				[
					'itemscope' => '',
		...
		<div <?php echo $this->get_render_attribute_string( 'uael-reviews-parent-schema' ); ?>>
			<div <?php echo $this->get_render_attribute_string( 'uael-reviews-parent-data' ); ?> <?php echo $this->get_carousel_attr(); ?>>
				<?php
			...
			if ( 'yes' === $settings['rating_schema_switch'] ) {
				...
				<div class="elementor-screen-only" itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating">
					<span itemprop="ratingValue" class="elementor-screen-only"><?php echo $aggregate; ?></span><span itemprop="reviewCount" class="elementor-screen-only"><?php echo $aggregate_count; ?></span>
				</div>
			<?php } ?>
		</div>
		<?php
