 ...
 * Plugin updates
 */
woothemes_queue_update( plugin_basename( __FILE__ ), 'e38ad13a5aaec7860df698cbad82c175', '1865835' );
...
final class Storefront_Powerpack {
	...
	public function includes() {
		...
		if ( 'storefront' === get_option( 'template' ) && apply_filters( 'storefront_powerpack_enabled', true ) ) {
			...
			// Mega Menus
			if ( ! class_exists( 'Storefront_Mega_Menus' ) && apply_filters( 'storefront_powerpack_mega_menus_enabled', true ) ) {
				include_once( 'includes/customizer/mega-menus/mega-menus.php' );
			}

			// Parallax Hero
			if ( ! class_exists( 'Storefront_Parallax_Hero' ) && apply_filters( 'storefront_powerpack_parallax_hero_enabled', true ) ) {
				include_once( 'includes/customizer/parallax-hero/customizer.php' );
				include_once( 'includes/customizer/parallax-hero/frontend.php' );
			}
			...
			if ( class_exists( 'WooCommerce' ) ) {
				...
				// Pricing tables
				if ( ! class_exists( 'Storefront_Pricing_Tables' ) && apply_filters( 'storefront_powerpack_pricing_tables_enabled', true ) ) {
					include_once( 'includes/customizer/pricing-tables/customizer.php' );
					include_once( 'includes/customizer/pricing-tables/frontend.php' );
					include_once( 'includes/customizer/pricing-tables/admin.php' );
				}

				// Reviews
				if ( ! class_exists( 'Storefront_Reviews' ) && apply_filters( 'storefront_powerpack_reviews_enabled', true ) ) {
					include_once( 'includes/customizer/reviews/customizer.php' );
					include_once( 'includes/customizer/reviews/frontend.php' );
					include_once( 'includes/customizer/reviews/admin.php' );
				}

				// Product Hero
				if ( ! class_exists( 'Storefront_Product_Hero' ) && apply_filters( 'storefront_powerpack_product_hero_enabled', true ) ) {
					include_once( 'includes/customizer/product-hero/customizer.php' );
					include_once( 'includes/customizer/product-hero/frontend.php' );
				}

				// Blog
				if ( ! class_exists( 'Storefront_Blog_Customiser' ) && apply_filters( 'storefront_powerpack_blog_customizer_enabled', true ) ) {
					include_once( 'includes/customizer/blog/customizer.php' );
					include_once( 'includes/customizer/blog/frontend.php' );
				}
			}
		}
	}
	...
	public function import_existing_storefront_extension_settings() {
		 ...
		 * Powerpack 2.0.
		 *
		 * Checks whether the extensions merged in Powerpack 2.0 have previously been installed and
		 * changes the value of some options.
		 */

		// Parallax Hero.
		if ( get_option( 'storefront-parallax-hero-version' ) && ! get_theme_mod( 'sph_hero_enable' ) ) {
			set_theme_mod( 'sph_hero_enable', 'enable' );
		}

		// Product Hero.
		if ( get_option( 'storefront-product-hero-version' ) && ! get_theme_mod( 'sprh_enable' ) ) {
			set_theme_mod( 'sprh_enable', 'enable' );
		}

		// Reviews.
		if ( get_option( 'storefront-reviews-version' ) && ! get_theme_mod( 'storefront_reviews_enable' ) ) {
			set_theme_mod( 'storefront_reviews_enable', 'enable' );
		}
	}
