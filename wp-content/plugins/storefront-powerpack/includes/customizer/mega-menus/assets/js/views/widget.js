	...
	api.SMM.WidgetView = wp.Backbone.View.extend({
		...
		toggleForm: function() {
			...
			if ( 0 === $widget.find( '#smm-save-widget' ).children().length ) {
				...
				// WordPress 4.9 - Render media widgets
				if ( wp.mediaWidgets.widgetControls[ this.id ] ) {
					wp.mediaWidgets.widgetControls[ this.id ].render();
				}
			...
			if ( ! $widget.hasClass( 'smm-widget-content-visible' ) ) {
				...
				if ( width && 0 < width ) {
					...
					if ( widgetRightEdge >= configuratorRightEdge ) {
						$widget.addClass( 'smm-widget-content-overflow-right' );
					} else if ( $( 'body' ).hasClass( 'rtl' ) && ( ( $widget.offset().left - width ) <= $configurator.offset().left ) ) {
						$widget.addClass( 'smm-widget-content-overflow-left' );
					}
				}
				...
				*/
				...
				$widget.animate({ 'height': '40' }, 200, function() {
					...
					$widget.removeClass( 'smm-widget-content-overflow-right' );
					$widget.removeClass( 'smm-widget-content-overflow-left' );
					$widget.width( 'auto' );
				});
			}
		},

		doWidgetUpdate: function() {
			var self = this,
				$widgetRoot,
				$widgetInside;
